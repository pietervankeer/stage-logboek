...
def get_charset_collation():
    ...
    output = exec_query("select SCHEMA_NAME, DEFAULT_CHARACTER_SET_NAME, DEFAULT_COLLATION_NAME from information_schema.schemata;")
    # test info
    output_lines = open_file(test_input_db_info, True).readlines()
    for line in output_lines:
        output_lines[output_lines.index(line)] = (" ".join(output.split())).replace(" ",",")+"\n"
    file_db_info.writelines(
        output_lines
    )
    # close file
    file_db_info.close()
...
def get_lv_sizes():
    ...
    arr_lv_sizes_lines = open_file(test_input_lv_size, True).readlines()
    
    for line in arr_lv_sizes_lines:
        if arr_lv_sizes_lines.index(line) == 0:
            arr_lv_sizes_lines[arr_lv_sizes_lines.index(line)] = line.replace("LV", "SCHEMA_NAME")
            
        arr_lv_sizes_lines[arr_lv_sizes_lines.index(line)] = (" ".join(arr_lv_sizes.split())).replace(" ", ",").replace("lv_", "") + "\n"
    # write lines in file
    file_lv_size.writelines(        
        arr_lv_sizes_lines
    )
    # close file
    file_lv_size.close()
...
def join_inputs:
    ...
    # Join data
    data_joined = pd.merge(data1, data2, 
                   on='SCHEMA_NAME', 
                   how='left')
                   
    file_db.write(
        data_joined.to_csv(index = False)
    )
    file_db.close()
def generate_output:
